...
func WhoisReads() []ReadSpec {
	...
	return []ReadSpec{
		...
		whoisSpec("poc", "poc", joinFields([]Field{required(text("handle", "handle")), text("first_name", "firstName"), text("last_name", "lastName"), text("company_name", "companyName"), text("poc_type", "pocType/type"), text("status", "status/code"), {Name: "is_role_account", Path: "isRoleAccount", Kind: BoolKind}, stringsField("emails", "emails/email"), objects("phones", "phones/phone", text("number", "number"), text("type", "type/code"))}, whoisAddressFields)),
		...
		whoisSpec("delegation", "delegation", []Field{required(text("name", "name")), stringsField("nameservers", "nameservers/nameserver"), objects("ds_records", "delegationKeys/delegationKey", required(integer("key_tag", "keyTag")), required(integer("algorithm", "algorithm")), required(integer("digest_type", "digestType")), required(text("digest", "digest")))}),
	}
}
