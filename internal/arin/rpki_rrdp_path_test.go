...
func TestRPKIRRDPPathPreflight(t *testing.T) {
	...
	for _, certs := range [][]*x509.Certificate{nil, {f.certs[0]}, {f.certs[0], nil, f.certs[2]}} {
		if _, err := client.RetrievePath(context.Background(), privateExchangeDir(t), certs, []string{"https://repo.example/0", "https://repo.example/1"}, cmsTrustNow()); err == nil {
			t.Fatal("invalid chain accepted")
		}
	}
	if _, err := client.RetrievePath(context.Background(), privateExchangeDir(t), f.certs, []string{"https://repo.example/0", "http://repo.example/1"}, cmsTrustNow()); err == nil {
