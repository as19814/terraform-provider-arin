...
func TestIRRRouteReadAndDeleteErrorsPreserveState(t *testing.T) {
	for _, status := range []int{404, 403, 429, 500} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			...
			r := &irrRouteResource{client: client}
			ctx := context.Background()
			var schemaResponse resource.SchemaResponse
			r.Schema(ctx, resource.SchemaRequest{}, &schemaResponse)
			initial := tfsdk.State{Schema: schemaResponse.Schema}
			model, d := irrRouteState(ctx, &arin.IRRRoute{Prefix: "192.0.2.0/24", OriginAS: "AS64496", NetHandle: "NET-EXAMPLE-1", OrgHandle: "EXAMPLE-1", Description: []string{"Example"}})
			if d.HasError() {
				t.Fatal(d)
			}
