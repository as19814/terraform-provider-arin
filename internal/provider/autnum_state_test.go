...
func TestAutnumReadAndDeleteErrorsPreserveState(t *testing.T) {
	for _, status := range []int{404, 403, 429, 500} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			...
			r := &autnumResource{client: client}
			ctx := context.Background()
			var schemaResponse resource.SchemaResponse
			r.Schema(ctx, resource.SchemaRequest{}, &schemaResponse)
			initial := tfsdk.State{Schema: schemaResponse.Schema}
			model, d := autnumState(ctx, &arin.Autnum{Name: "AS64496", OrgHandle: "EXAMPLE-1", Description: []string{"Example"}})
			if d.HasError() {
				t.Fatal(d)
			}
