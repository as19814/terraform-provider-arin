...
func TestDelegationReadAndDeleteErrorsPreserveState(t *testing.T) {
	for _, status := range []int{404, 403, 429, 500} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			...
			r := &delegationResource{client: client}
			ctx := context.Background()
			var schemaResponse resource.SchemaResponse
			r.Schema(ctx, resource.SchemaRequest{}, &schemaResponse)
			initial := tfsdk.State{Schema: schemaResponse.Schema}
			model, d := delegationState(ctx, &arin.Delegation{Name: "2.0.192.in-addr.arpa."})
			if d.HasError() {
				t.Fatal(d)
			}
...
func TestDelegationDeleteVerifiesClearing(t *testing.T) {
	ctx := context.Background()
	...
	model, d := delegationState(ctx, &arin.Delegation{Name: "2.0.192.in-addr.arpa.", Nameservers: []arin.DelegationNameserver{{Name: "ns.example.net"}}})
	if d.HasError() {
		t.Fatal(d)
	}
	if d = initial.Set(ctx, &model); d.HasError() {
	...
	response := resource.DeleteResponse{State: initial}
	r.Delete(ctx, resource.DeleteRequest{State: initial}, &response)
	if !response.Diagnostics.HasError() || !response.State.Raw.Equal(initial.Raw) {
		t.Fatal("unconfirmed clearing must retain state and report an error")
	}
}
