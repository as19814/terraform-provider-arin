...
func TestASSetReadAndDeleteErrorsPreserveState(t *testing.T) {
	for _, status := range []int{404, 403, 429, 500} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			...
			r := &asSetResource{client: client}
			ctx := context.Background()
			var schemaResponse resource.SchemaResponse
			r.Schema(ctx, resource.SchemaRequest{}, &schemaResponse)
			initial := tfsdk.State{Schema: schemaResponse.Schema}
			model, d := asSetState(ctx, &arin.ASSet{Name: "AS-EXAMPLE", OrgHandle: "EXAMPLE-1", Description: []string{"Example"}})
			if d.HasError() {
				t.Fatal(d)
			}
