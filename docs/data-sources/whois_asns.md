---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "arin_whois_asns Data Source - arin"
subcategory: ""
description: |-
  Search public Whois-RWS asns using one or more documented filters. References are returned by default; show_details requests full records. Typed fields and complete XML are retained. No API key is sent. Partial results and referrals are errors; recognized no-results responses produce an empty list.
---

# arin_whois_asns (Data Source)

Search public Whois-RWS asns using one or more documented filters. References are returned by default; show_details requests full records. Typed fields and complete XML are retained. No API key is sent. Partial results and referrals are errors; recognized no-results responses produce an empty list.

## Example Usage

```terraform
# Generated by go generate.
data "arin_whois_asns" "example" {
  filters      = { "handle" : "AS64496" }
  show_details = false
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `filters` (Map of String) Nonempty map of search predicates. Allowed keys: handle, name. Predicates are combined with AND. Values match case-insensitively; a single trailing * requests a prefix match. Unknown keys, empty values and other wildcard positions are rejected locally.

### Optional

- `show_details` (Boolean) Ask ARIN to expand related information inline. Complete XML preserves extra records; any nested truncation is rejected. Defaults to `false`.

### Read-Only

- `asns` (Attributes List) Matching records ordered deterministically. Reference responses omit detail-only fields; show_details requests full records. Recognized no-results responses produce an empty list. Truncation is an error. (see [below for nested schema](#nestedatt--asns))
- `whois_xml` (String) Complete search response XML. Null for a recognized Whois no-results HTTP 404. Links are not followed.

<a id="nestedatt--asns"></a>
### Nested Schema for `asns`

Read-Only:

- `comments` (List of String) Ordered lines of comments.
- `end_asn` (Number) end asn returned by ARIN.
- `handle` (String) handle returned by ARIN.
- `name` (String) name returned by ARIN.
- `org_handle` (String) org handle returned by ARIN.
- `rdap_ref` (String) rdap ref returned by ARIN.
- `ref` (String) ref returned by ARIN.
- `reference_name` (String) reference name returned by ARIN.
- `registration_date` (String) registration date returned by ARIN.
- `start_asn` (Number) start asn returned by ARIN.
- `update_date` (String) update date returned by ARIN.
